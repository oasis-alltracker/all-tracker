...
module.exports.handler = async (event, context, callback) => {
  ...
  if(event.httpMethod == "GET") {
    response = await getHabitStatuses.getHabitStatuses(user, event.queryStringParameters);
  }
  else if(event.httpMethod == "PUT") {
    response = await updateHabitStatus.updateHabitStatus(user, habitStatusID, JSON.parse(event.body));
  }
  else if(event.httpMethod == "POST") {
    response = await createHabitStatus.createHabitStatus(user, JSON.parse(event.body));
  }
