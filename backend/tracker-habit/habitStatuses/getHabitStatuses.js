    ...
    async getHabitStatuses(user, queryStringParameters) {
        try {
            ...
            if(queryStringParameters.dateStamp) {
                habitStatuses = await this.getStatusesForOneDay(user.email, queryStringParameters.dateStamp);
            }
            else if(queryStringParameters.startDate && queryStringParameters.endDate){
                habitStatuses = await this.getStatusesForMulitpleDays(user.email, queryStringParameters.startDate, queryStringParameters.endDate);
            }
            else{
