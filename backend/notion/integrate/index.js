...
async function integrate(user, code) {
    ...
    if(!integrationMetadata.access_token || !integrationMetadata.duplicated_template_id) {
    ...
    await saveDatabaseID(user.email, integrationMetadata.access_token, "notion-token");
                        ...
                        for(databaseChild of childPageResponse.results){
                                ...
                                for(column of dietColumnListRes.results) {
                                    ...
                                    for(dietDB of dietColumnResponse.results) {
                                        if(dietDB.type == "child_database") {
                                            ...
                                            if(databaseChildResponse.child_database.title == "Food Items"){
                                                await saveDatabaseID(user.email, databaseChildResponse.id, "food-items");
                                            }
                                            await new Promise(r => setTimeout(r, 350));  
                                        }
                                    }
                            ...
                            if(databaseChild.type == "child_database") {
                                ...
                                if(databaseChildResponse.child_database.title == "To-dos"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "to-dos");
                                }
                                else if(databaseChildResponse.child_database.title == "Daily Journal"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "daily-journal");
                                }
                                else if(databaseChildResponse.child_database.title == "Habits"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "habits");
                                }
                                else if(databaseChildResponse.child_database.title == "Meal Entries"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "meal-entries");
                                }
                                else if(databaseChildResponse.child_database.title == "Food Items"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "food-items");
                                }
                                else if(databaseChildResponse.child_database.title == "Workouts"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "workouts");
                                }
                                else if(databaseChildResponse.child_database.title == "Check-ins"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "check-ins");
                                }
                                else if(databaseChildResponse.child_database.title == "Sleep Reports"){
                                    await saveDatabaseID(user.email, databaseChildResponse.id, "sleep-reports");
                                }
                                await new Promise(r => setTimeout(r, 350));
                            }
                        }
