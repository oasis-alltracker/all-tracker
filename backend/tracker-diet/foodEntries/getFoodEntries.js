    ...
    async getFoodEntries(user, queryStringParameters) {
        try {
            ...
            if(queryStringParameters.dateStamp) {
                foodEntries = await this.getEntriesForOneDay(user.email, queryStringParameters.dateStamp);
            }
            else if(queryStringParameters.startDate && queryStringParameters.endDate){
                foodEntries = await this.getEntriesForMulitpleDays(user.email, queryStringParameters.startDate, queryStringParameters.endDate);
            }
            else{
