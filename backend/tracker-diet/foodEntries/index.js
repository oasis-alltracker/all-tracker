...
module.exports.handler = async (event, context, callback) => {
  ...
  if(event.httpMethod == "GET") {
    response = await getFoodEntries.getFoodEntries(user, event.queryStringParameters);
  }
  else if(event.httpMethod == "PUT") {
    response = await updateFoodEntry.updateFoodEntry(user, foodEntryID, JSON.parse(event.body));
  }
  else if(event.httpMethod == "POST") {
    response = await createFoodEntry.createFoodEntry(user, JSON.parse(event.body));
  }
