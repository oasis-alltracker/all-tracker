class UpdateFoodItem {
  ...
  async updateItem(email, foodItemID, foodEntry) {
    ...
    const expression =
      "SET #name = :name, #caloriesPerMeasure = :caloriesPerMeasure, #proteinsPerMeasure = :proteinsPerMeasure, #fatsPerMeasure = :fatsPerMeasure, #servingOption = :servingOption, #foodIDs = :foodIDs";
    const names = {
      "#name": "name",
