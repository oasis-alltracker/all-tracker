...
module.exports.handler = async (event, context, callback) => {
  ...
  if(event.httpMethod == "GET") {
    response = await getWorkoutEntries.getWorkoutEntries(user, event.queryStringParameters);
  }
  else if(event.httpMethod == "PUT") {
    response = await updateWorkoutEntry.updateWorkoutEntry(user, workoutEntryID, JSON.parse(event.body));
  }
  else if(event.httpMethod == "POST") {
    response = await createWorkoutEntry.createWorkoutEntry(user, JSON.parse(event.body));
  }
  else if(event.httpMethod == "DELETE") {
    response = await deleteWorkoutEntry.deleteWorkoutEntry(user, workoutEntryID);
  }
