    ...
    async getWorkoutEntries(user, queryStringParameters) {
        try {
            ...
            if(queryStringParameters.dateStamp) {
                workoutEntries = await this.getEntriesForOneDay(user.email, queryStringParameters.dateStamp);
            }
            else if(queryStringParameters.startDate && queryStringParameters.endDate){
                workoutEntries = await this.getEntriesForMulitpleDays(user.email, queryStringParameters.startDate, queryStringParameters.endDate);
            }
            else{
