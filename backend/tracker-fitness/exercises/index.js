...
module.exports.handler = async (event, context, callback) => {
  ...
  if(event.httpMethod == "GET") {
    response = await getExercises.getExercises(user, event.queryStringParameters);
  }
  else if(event.httpMethod == "PUT") {
    response = await updateExercise.updateExercise(user, exerciseID, JSON.parse(event.body));
  }
  else if(event.httpMethod == "POST") {
    response = await createExercise.createExercise(user, JSON.parse(event.body));
  }
