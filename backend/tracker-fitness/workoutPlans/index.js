...
module.exports.handler = async (event, context, callback) => {
  ...
  if(event.httpMethod == "GET") {
    ...
    response = await updateWorkoutPlan.updateWorkoutPlan(user, workoutPlanID, JSON.parse(event.body));
  }
  else if(event.httpMethod == "POST") {
    response = await createWorkoutPlan.createWorkoutPlan(user, JSON.parse(event.body));
  }
