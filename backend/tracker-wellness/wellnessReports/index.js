...
module.exports.handler = async (event, context, callback) => {
  ...
  if(event.httpMethod == "GET") {
    response = await getWellnessReports.getWellnessReports(user, event.queryStringParameters);
  }
  else if(event.httpMethod == "PUT") {
    response = await updateWellnessReport.updateWellnessReport(user, wellnessReportID, JSON.parse(event.body));
  }
  else if(event.httpMethod == "POST") {
    response = await createWellnessReport.createWellnessReport(user, JSON.parse(event.body));
  }
  else if(event.httpMethod == "DELETE") {
    response = await deleteWellnessReport.deleteWellnessReport(user, wellnessReportID);
  }
