    ...
    async getSleepReports(user, queryStringParameters) {
        try {
            ...
            if(queryStringParameters.dateStamp) {
                sleepReports = await this.getReportsForOneDay(user.email, queryStringParameters.dateStamp);
            }
            else if(queryStringParameters.startDate && queryStringParameters.endDate){
                sleepReports = await this.getReportsForMulitpleDays(user.email, queryStringParameters.startDate, queryStringParameters.endDate);
            }
            else{
